...
CH264StdStream::CH264StdStream(void)
{
...
void CH264StdStream::Release(void)
{
...
void CH264StdStream::ShutDown(void)
{
	blContinuePlay = False;
	if (env != NULL)
	{
...
void CH264StdStream::configH264Stream(char *pParm,int inType,int bUseTCP)
{
	if (pParm != NULL)
	{
...
int CH264StdStream::createRTSP(void)
{
	numReceiverByte = 0;
	kBytes = 0;
	numReceiverByteA = 0;
...
void CH264StdStream::setupStream(void)
{
	if (setupIter == NULL)
	{
...
void CH264StdStream::playStream(void)
{
	if (duration == 0)
	{
		if (scale > 0)
...
void CH264StdStream::insertSink(void)
{
	MediaSubsessionIterator iter(*session);
	MediaSubsession *subsession;
	while((subsession = iter.next()) != NULL)
...
//		rtspClient->sendPlayCommand(*subsession, continueAfterPLAY, initialSeekTime,endtime,scale, NULL);
	...
	if (pcmSink || fileSink)
	{
		rtspClient->sendPlayCommand(*session,continueAfterPLAY, initialSeekTime,0,scale, NULL);
	}
}

void CH264StdStream::StartStreamThd()
{
...
{
	CH264StdStream * h264Stream = (CH264StdStream*)lpParam;
...
{
	CH264StdStream * h264stream = (CH264StdStream *)rtspClient->fobjPtr;
	if (resultCode!= 0)
	{
...
{
	CH264StdStream * h264stream = (CH264StdStream *)clientData;
