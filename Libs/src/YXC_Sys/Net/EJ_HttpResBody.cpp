...
namespace EJDev
{
	...
	EJ_Status HttpResponseBody::BuildHttpResponse(const HttpResponse* response_data, EJ_Buffer* buffer)
	{
		char szTemp1[4096];
		int nSize;

		nSize = ejcwrap_snprintf(szTemp1, 4095, 
			"HTTP/1.0 %d %s\r\n"
		...
		euint32_t nTotal = nSize + response_data->content_length;
		_ECHK_MAL_ARR_R2(buffer->pBuffer, ebyte_t, nTotal);

		memcpy(buffer->pBuffer, szTemp1, nSize);
		memcpy((ebyte_t*)buffer->pBuffer + nSize, response_data->content_data, response_data->content_length);
		buffer->stBufSize = nTotal;
		return EJ_ERC_SUCCESS;
	}

	EJ_Status HttpResponseBody::BuildStatusResponse(int code, const char* code_desc, const char* response_text, 
		int close_connection, EJ_Buffer* buffer)
	{
	...
	void HttpResponseBody::FreeResponseBuffer(EJ_Buffer* buffer)
	{
		free(buffer->pBuffer);
	}

	EJ_Status HttpResponseBody::_AddHeaderDesc(const char* key_begin, const char* key_end, const char* val_begin, const char* val_end)
	{
		...
		else if (key_len == EJ_STRING_ARR_LEN(kTransfer) && strncmp(key_begin, kTransfer, key_len) == 0)
		{
	...
	EJ_Status HttpResponseBody::_ParseHeaderExtenstions(const char* headers, int length)
	{
	...
	EJ_Status HttpResponseBody::_ParseCodeAndDesc(int len)
	{
		const char* begin = (char*)this->_pTempData;
		const char* kSpace = strchr(begin, ' ');
		_EJ_CHECK_REPORT_NEW_RET(kSpace != NULL, EJ_ERC_NET_INVALID_PROTOCOL, EC("strchr for code and desc failed"));
		
		const char* kSpace2 = strchr(kSpace + 1, ' ');
		_EJ_CHECK_REPORT_NEW_RET(kSpace2 != NULL, EJ_ERC_NET_INVALID_PROTOCOL, EC("strchr for code and desc failed"));

		euint32_t uCountCode = kSpace2 - kSpace - 1, uCountDesc = len - (kSpace2 - begin);
		_ECHK_EAL_STRA_R1(resCodeStr, this->_emp, uCountCode);
		strncpy(resCodeStr, kSpace + 1, uCountCode);
		resCodeStr[uCountCode] = 0;

		this->_responseCode = atoi(resCodeStr);
		
		_ECHK_EAL_STRA_R1(resDescStr, this->_emp, uCountDesc);
	...
	EJ_Status HttpResponseBody::_ParseHeader()
	{
		...
		if (header_found)
		{
			...
			EJ_Status rc = this->_ParseCodeAndDesc(len);
			_EJ_CHECK_RC_RET(rc);
	...
	EJ_Status HttpResponseBody::_ParseChunked()
	{
		while (this->_stCbChunkPos < this->_stCbTempData)
		{
			...
			int chunk_total_size = chunk_size + chunk_size_len + 2 * EJ_STRING_ARR_LEN(kLineTerminator);
			if (chunk_total_size <= this->_stCbTempData - index) /* Read this chunk out. */
			{
				...
				memmove(this->_pTempData + this->_stCbHttpHeader + this->_stCbDataInChunk, 
					header_found + EJ_STRING_ARR_LEN(kLineTerminator), chunk_size);
	...
	ebool_t HttpResponseBody::IsResponseReceived() const
	{
		if (!this->_chunked)
		{
	...
	EJ_Status HttpResponseBody::AppendPacket(const void* data, euint32_t len)
	{
		_EJ_CHECK_REPORT_NEW_RET(!this->IsResponseReceived(), EJ_ERC_INVALID_STATUS, EC("Response not clear before read"));

		EJ_Status rc = EJ_MMCExpandBuffer((void**)&this->_pTempData, &this->_stCbTempBuf, this->_stCbTempData + len + sizeof(char));
		_EJ_CHECK_RC_RET(rc);
		...
		if (!this->_bHeaderParsed)
		{
			_EJ_CHECK_REPORT_NEW_RET(this->_stCbTempData < (1 << 20), EJ_ERC_BUFFER_NOT_ENOUGH, EC("Not a valid http response header(%d)."), 
				this->_stCbTempData);
		}
		else
		{
